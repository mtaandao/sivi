...
$loop = new MN_Query(array('post_type' => 'portfolio', 'posts_per_page' => $items_per_page[0])); 
?>
<div class="container">
	...
	<div class="row">
		...
		<div class="<?php if ($weblusive_sidebar_pos == 'full'):?>col-md-12<?php else:?>col-md-8<?php endif?> main-content">
				...
				<div id="portfolio-filters" class="portfolio-filters">
					<ul data-option-key="filter" class="folio-filter xtra <?php if (!$paginationEnabled):?><?php echo 'non-paginated' ?><?php endif?>">
						<?php if ($paginationEnabled):?>
							<li><a href="<?php echo esc_url(get_page_link($pageId)) ?>" class="button solid-button white-purple small"><?php _e('All', 'Sivi')?></a></li>
							<?php 
							...
							?>
						<?php else: ?>
							<li><a href="#filter" class="button solid-button white-purple small" data-filter="*"><?php _e('All', 'Sivi')?></a></li>
							<?php 
								$cats = get_post_meta($post->ID, "_page_portfolio_cat", $single = true);                                                 
			...
			<div class="portfolio" id="portfolio">
						<?php if( $cats == '' ): ?>
							<p><?php _e('No categories selected. To fix this, please login to your MN Admin area and set
						...
						<?php else: ?>		
							<?php 
							 ...
							 <?php if (!empty($portfolio_posts_to_query)):			
								$mn_query = new MN_Query( array( 'post_type' => 'portfolio', 'orderby' => 'menu_order', 'order' => 'ASC', 'post__in' => $portfolio_posts_to_query, 'paged' => $paged, 'shomnosts' => $items_per_page[0] ) ); 
								
								if ($mn_query->have_posts()):  ?>
								<?php while ($mn_query->have_posts()) : 							
									$mn_query->the_post();
			...
			<?php if ($paginationEnabled ):?>
				<?php if ( $mn_query->max_num_pages > 1 ): ?>
					<div class="row">
						<div class="col-md-12  pagination-wrapper">
							<?php include(sivi_PLUGINS . '/mn-pagenavi.php' ); mn_pagenavi(); ?> 
