<?php echo '<style type="text/css">' ;?>
	...
	?>
		...
		.solid-button.purple,.social-icon:hover, .tweet .twitter-item i.fa-twitter, .progress-bar-sivi, .btn.purple, .tabs-shortcode ul.nav-tabs li a,
		.service:hover, .icon-box:hover .icon, .experience-block .meta .year, .solid-button.white-purple:hover, .testimonial-slider .sep, 
		.owl-theme .owl-dots .owl-dot.active span, .owl-theme .owl-dots .owl-dot:hover span, #mn-calendar caption, .widget-tab ul.nav-tabs li a,
		...
		input[type='submit'], .mncf7-submit{border-color:<?php echo $maincolor?> !important}
	...
	<?php endif?>
		...
		<?php endif; ?>		
		<?php
		function sivi_enqueue_font ( $got_font) {
			if ($got_font) {
				...
				if( $font_type == 'non-google' ){
					...
					mn_enqueue_style( $font_name , $protocol.'://fonts.googleapis.com/earlyaccess/'.$font_name_link.'.css');
				}else{
					$font_name_link = str_replace (" ","+", $font_pieces[0] );
					$font_variants = str_replace ("|",",", $font_pieces[1] );
					$protocol = is_ssl() ? 'https' : 'http';
					mn_enqueue_style( $font_name , $protocol.'://fonts.googleapis.com/css?family='.$font_name_link.':'.$font_variants);
				}
				return $font_name;      
			}
		...
		body, p, li, .acc-shortcode .panel-body{
			<?php $maintyp=get_theme_mod('main_typ_font'); echo (isset($maintyp) && !empty($maintyp) ) ? 'font-family:'.sivi_enqueue_font($maintyp).' !important;' : '' ?>
		...
		.header .bottom .title a{
			<?php $logtyp=get_theme_mod('log_typ_font'); echo (isset($logtyp) && !empty($logtyp) ) ? 'font-family:'.sivi_enqueue_font($logtyp).' !important;' : '' ?>
		...
		.main-nav ul>li>a{
			<?php $navtyp=get_theme_mod('nav_typ_font'); echo (isset($navtyp) && !empty($navtyp) ) ? 'font-family:'.sivi_enqueue_font($navtyp).' !important;' : '' ?>
		...
		h1{
			<?php $h1typ=get_theme_mod('h1_typ_font'); echo (isset($h1typ) && !empty($h1typ) ) ? 'font-family:'.sivi_enqueue_font($h1typ).' !important;' : '' ?>
		...
		h2{
			<?php $h2typ=get_theme_mod('h2_typ_font'); echo (isset($h2typ) && !empty($h2typ) ) ? 'font-family:'.sivi_enqueue_font($h2typ).' !important;' : '' ?>
		...
		h3{
			<?php $h3typ=get_theme_mod('h3_typ_font'); echo (isset($h3typ) && !empty($h3typ) ) ? 'font-family:'.sivi_enqueue_font($h3typ).' !important;' : '' ?>
		...
		h4{
			<?php $h4typ=get_theme_mod('h4_typ_font'); echo (isset($h4typ) && !empty($h4typ) ) ? 'font-family:'.sivi_enqueue_font($h4typ).' !important;' : '' ?>
		...
		h5{
			<?php $h5typ=get_theme_mod('h5_typ_font'); echo (isset($h5typ) && !empty($h5typ) ) ? 'font-family:'.sivi_enqueue_font($h5typ).' !important;' : '' ?>
		...
		h6{
			<?php $h6typ=get_theme_mod('h6_typ_font'); echo (isset($h6typ) && !empty($h6typ) ) ? 'font-family:'.sivi_enqueue_font($h6typ).' !important;' : '' ?>
		...
		h2.onetitle{
			<?php $ptittyp=get_theme_mod('ptit_typ_font'); echo (isset($ptittyp) && !empty($ptittyp) ) ? 'font-family:'.sivi_enqueue_font($ptittyp).' !important;' : '' ?>
		...
		h2.page-title{
			<?php $altptittyp=get_theme_mod('altptit_typ_font'); echo (isset($altptittyp) && !empty($altptittyp) ) ? 'font-family:'.sivi_enqueue_font($altptittyp).' !important;' : '' ?>
		...
		p.onesubtitle,  p.tagline{
			<?php $tagtyp=get_theme_mod('tag_typ_font'); echo (isset($tagtyp) && !empty($tagtyp) ) ? 'font-family:'.sivi_enqueue_font($tagtyp).' !important;' : '' ?>
		...
		.sidebar-widget h4.widget-title{
			<?php $wtittyp=get_theme_mod('wtit_typ_font'); echo (isset($wtittyp) && !empty($wtittyp) ) ? 'font-family:'.sivi_enqueue_font($wtittyp).' !important;' : '' ?>
		...
		.footer h4.footer-title{
			<?php $ftittyp=get_theme_mod('ftit_typ_font'); echo (isset($ftittyp) && !empty($ftittyp) ) ? 'font-family:'.sivi_enqueue_font($ftittyp).' !important;' : '' ?>
