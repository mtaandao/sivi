...
if(isset( $get_meta['_weblusive_post_head'][0]) && $get_meta['_weblusive_post_head'][0] != 'none' ):
	...
	if(isset( $get_meta['_weblusive_post_head'][0]) && $get_meta['_weblusive_post_head'][0] == 'thumb' || (isset( $get_meta['_weblusive_post_head'][0]) && empty( $get_meta['_weblusive_post_head'][0] ) && weblusive_get_option( 'post_featured' ) ) ){
		...
		$image_url = mn_get_attachment_image_src($image_id,'full');  
			...
			<div class="row">
				<div class="col-md-12">
					<div class="portfolio-media">
						...
						<?php else: ?>
							<p class="label label-warning"><?php _e('Vimeo video chosen from post head options but no video ID provided. Please make sure to provide a video ID to display it.', 'Sivi')?></p>
						<?php endif?>
					</div>
				</div>
			</div>
				...
				<div class="col-md-12">
					<div class="portfolio-media">
						<div class="flex-video youtube">
							...
							<?php else: ?>
								<p><?php _e('Youtube video was chosen from post head options but no video ID provided. Please make sure to provide a video ID  to display it.', 'Sivi')?></p>
							<?php endif?>
						</div>
					</div>
				</div>
	...
	elseif(isset( $get_meta['_weblusive_post_head'][0]) && $get_meta['_weblusive_post_head'][0] == 'slider' && !empty( $get_meta['_weblusive_post_slider'][0] ) ){
		...
		$custom_slider = new MN_Query( $custom_slider_args );
					...
					<div class="homeSlider flexslider row m0">
						<ul class="slides nav">
							<?php $counter=0; foreach( $slider as $slide ): 
								...
								$image =  mn_get_attachment_image_src( $slide['id'], 'full'); 
			...
			<section id="slider_blocks" class="empty_slider">
				<div class="col-md-12 text-center">
					<p class="label label-danger">
						...
						<?php _e('No slider items were found in the selected slider. Please make sure to create some via "Slider" section in your admin panel.', 'Sivi');?>
					</p>
				</div>
			</section>
		<?php endif;?>
