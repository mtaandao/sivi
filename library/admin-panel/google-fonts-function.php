...
function weblusive_enqueue_font ( $got_font) {
	...
	if ($got_font) {
		...
		if( $font_type == 'non-google' ){
			...
			mn_enqueue_style( $font_name , $protocol.'://fonts.googleapis.com/earlyaccess/'.$font_name);
			...
			mn_enqueue_style( $font_name , $protocol.'://fonts.googleapis.com/css?family='.$font_name.':'.$font_variants.$char_set);
