...
if ( isset( $_GET['page'] ) && $_GET['page'] == 'panel' ) {
	function weblusive_enqueue_pointer_script_style( $hook_suffix ) {
		...
		$dismissed_pointers = explode( ',', get_user_meta( get_current_user_id(), 'dismissed_mn_pointers', true ) );
...
add_action( 'mn_before_admin_bar_render', 'weblusive_admin_bar' );
...
function weblusive_admin_register() {
    ...
    mn_register_script( 'weblusive-admin-slider', get_template_directory_uri() . '/library/admin-panel/js/jquery.ui.slider.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-mouse', 'jquery-ui-sortable' ) , false , false );  
    mn_enqueue_script( 'weblusive-admin-tipsy', get_template_directory_uri() . '/library/admin-panel/js/tipsy.js', array( 'jquery' ) , false , false );  
	mn_register_script( 'weblusive-admin-main', get_template_directory_uri() . '/library/admin-panel/js/main.js', array( 'jquery' ) , false , false );  
    mn_register_script( 'weblusive-admin-colorpicker', get_template_directory_uri() . '/library/admin-panel/js/colorpicker.js', array( 'jquery' ) , false , false );  
	mn_register_script( 'weblusive-admin-helper', get_template_directory_uri() . '/library/admin-panel/js/helper.js', array( 'jquery' ) , false , false );  
	
	mn_register_style( 'weblusive-style', get_template_directory_uri().'/library/admin-panel/style.css', array(), '20120208', 'all' ); 

	if ( isset( $_GET['page'] ) && $_GET['page'] == 'panel' ) {

		mn_enqueue_script( 'weblusive-admin-colorpicker');  
		mn_enqueue_script( 'weblusive-admin-helper' );  
		mn_enqueue_script( 'weblusive-admin-slider' );  
	}
	
	mn_enqueue_script( 'weblusive-admin-main' );
	mn_enqueue_style( 'weblusive-style' );
...
function weblusive_replace_thickbox_text($translated_text, $text, $domain) { 
    if ('Insert into Post' == $text) { 
        ...
        $referer = strpos( mn_get_referer(), 'weblusive-settings' );
        if ( $referer != '' )
            return __('Use this image', 'Sivi' ); 
    }  
...
# Save Options
/*-----------------------------------------------------------------------------------*/
add_action('mn_ajax_test_theme_data_save', 'weblusive_save_ajax');
...
function weblusive_add_admin() {
	...
	//add_submenu_page('panel', theme_name, 'Pricing Tables', 'switch_themes', 'pricing_tables', 'pricing_tables');
...
?>
	...
	switch ( $value['type'] ) {
		...
		mn_editor(htmlspecialchars_decode( weblusive_get_option($value['id'])), $value['id'], array( 'textarea_name' => 'weblusive_options['.$value['id'].']', 'media_buttons' => false, 'textarea_rows' => 10 /* 'teeny' => true */) ); 
		?>
		<?php
		break;
