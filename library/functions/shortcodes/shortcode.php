...
function add_alc_custom_tinymce_plugin($plugin_array) {
	$plugin_array['SiviShortcodes'] = JS_PATH;
	return $plugin_array;
}
add_action('init', 'Sivi_addbuttons');

function sumtips_add_dfe_buttons($buttons)
{
	$buttons[] = 'separator'; //Add separator (optional)
	$buttons['SiviShortcodes'] = array(
		'title' => __('Sivi Shortcodes', "Sivi"), //Button Title
		'onclick' => "tinyMCE.execCommand('alc_sivi');", //Command to execute
		'both' => true // Show in visual mode. Set 'true' to show in both visual and HTML mode
	);
	
	return $buttons;
}
add_filter( 'mn_fullscreen_buttons', 'sumtips_add_dfe_buttons' );
	...
	$return='<div id="testimonials-'.$randomId.'" class="test-shortcode testimonial-slider owl-carousel '.$anim.' '.$class.'">';
					if( is_array( $GLOBALS['testimonials'] ) ){
						...
						if(function_exists('vc_map')){
							$limage = mn_get_attachment_image_src($testimonial['photo'], 'full');
							$imglink.=$limage[0];
						}else{
...
function alc_reveal($atts, $content=NULL){
    ...
    $out.='<div id="myModal'.$randomId.'"  class="modal fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false">
	  ...
	  $out.='<div class="modal-footer"><button class="btn" data-dismiss="modal" aria-hidden="true">'.__('Close', 'Sivi').'</button></div>';
         $out.='</div>
          </div>
	 </div>
	';	
							...
							<div class="media recent-post">';
								...
								elseif (( function_exists( 'get_post_format' ) && 'video' == get_post_format( $ps->ID ) )):
									...
									$return.='<div class="full-video">';
										if ($video):
											$post_embed = $mn_embed->run_shortcode('[embed width="500" height="400"]'.$video.'[/embed]'); 
										else:
								...
								$full_image = mn_get_attachment_image_src(get_post_thumbnail_id($ps->ID), 'full', false); 
								$argsThumb = array(
									'order'          => 'ASC',
									'posts_per_page'  => 99,
								...
								$return.='<ul class="bxslider">';
											...
											if ($attachments) {
												foreach ($attachments as $attachment) {
													$image = mn_get_attachment_url($attachment->ID, 'full', false, false);
								...
								$return.='<a href="' . get_permalink($ps->ID) . '" >
												<img src = "http://placehold.it/267x186" alt="' . __('No image', 'Sivi') . '" />
										</a>';
							endif;
...
function related_posts_shortcode( $atts ) {
	...
	if ( $dtwd_related_posts ) {
		foreach($dtwd_related_posts as $r) {
			...
			<div class="col-md-4 col-sm-4">
				<div class="related-post-media">';
					if ($video):
						$retval.='<div class="flex-video">'.$mn_embed->run_shortcode('[embed width="262" height="197"]'.$video.'[/embed]').'</div>';	
					else:	
						if ($thumbnail !== ''):
							$retval.=$thumbnail;
						else:	
							//$retval.='<img src = "http://placehold.it/550x403" alt="'.__('No Image', 'Sivi').'" />';
						endif;
					endif;		
				$retval .= '</div>
				<div class="related-post-content">
					<div class="related-post-title"><a href="'.get_permalink($r->ID).'">'.mntexturize($r->post_title).'</a></div>
		...
		$retval .= '<div class="col-md-12"><p>'.__('No related posts for this one', 'Sivi').'</div>';
	}
...
function alc_lightbox( $atts, $content = null ) {
	...
	$imglink='';
	if(function_exists('vc_map')){
		$limage = mn_get_attachment_image_src($thumbnail, 'full');
		$imglink.=$limage[0];
	}else{
		$imglink=$thumbnail;
	}
...
function mapme($attr) {
	...
	$imglink='';
	if(function_exists('vc_map')){
		$limage = mn_get_attachment_image_src($attr['markerimage'], 'full');
		$imglink.=$limage[0];
	}else{
...
function alc_regimage($atts, $content=NULL){
	...
	$imglink='';
		if(function_exists('vc_map')){
			$limage = mn_get_attachment_image_src($image, 'full');
			$imglink.=$limage[0];
		}else{
			$imglink=$image;
		}
...
function Sivi_one_whole( $atts, $content = null ) {
	return '<div class="col-md-12">' . do_shortcode($content) . '</div>';
}
add_shortcode('one_whole', 'Sivi_one_whole');

function Sivi_one_half( $atts, $content = null ) {

	return '<div class="col-md-6">' . do_shortcode($content) . '</div>';
}
add_shortcode('one_half', 'Sivi_one_half');


function Sivi_one_third( $atts, $content = null ) {
	return '<div class="col-md-4">' . do_shortcode($content) . '</div>';
}
add_shortcode('one_third', 'Sivi_one_third');


function Sivi_two_third( $atts, $content = null ) {
	return '<div class="col-md-8">' . do_shortcode($content) . '</div>';
}
add_shortcode('two_third', 'Sivi_two_third');


function Sivi_one_fourth( $atts, $content = null ) {

	return '<div class="col-md-3">' . do_shortcode($content) . '</div>';
}
add_shortcode('one_fourth', 'Sivi_one_fourth');

function Sivi_three_fourth( $atts, $content = null ) {
	return '<div class="col-md-9">' . do_shortcode($content) . '</div>';
}
add_shortcode('three_fourth', 'Sivi_three_fourth');

function Sivi_one_sixth( $atts, $content = null ) {
	return '<div class="col-md-2">' . do_shortcode($content) . '</div>';
}
add_shortcode('one_sixth', 'Sivi_one_sixth');

function Sivi_five_twelveth( $atts, $content = null ) {
	return '<div class="col-md-5">' . do_shortcode($content) . '</div>';
}
add_shortcode('five_twelveth', 'Sivi_five_twelveth');

function Sivi_seven_twelveth( $atts, $content = null ) {
	return '<div class="col-md-7">' . do_shortcode($content) . '</div>';
}
add_shortcode('seven_twelveth', 'Sivi_seven_twelveth');

function Sivi_five_sixth( $atts, $content = null ) {
	return '<div class="col-md-10">' . do_shortcode($content) . '</div>';
}
add_shortcode('five_sixth', 'Sivi_five_sixth');

function Sivi_row( $atts, $content = null ) {
   return '<div class="row">' . do_shortcode($content) . '</div>';
}
add_shortcode('row', 'Sivi_row');

function Sivi_inner_row( $atts, $content = null ) {
   return '<div class="row">' . do_shortcode($content) . '</div>';
}
add_shortcode('inner_row', 'Sivi_inner_row');
/************************************************/
