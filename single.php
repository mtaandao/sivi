						...
						<div class="single-blog-post post-format-<?php echo $format?>">
							...
							<header class="blog-header clearfix <?php echo $header_class; ?>">
								<div class="content">
									...
									<h4><?php the_title()?></h4>
									<span class="meta"><?php $meta = sivi_meta($post); echo $meta ?></span>
									<div class="footer">
										<p class="button-row blog-button-row">
											<?php				
											$prev_post = get_previous_post();
											if (!empty( $prev_post )): ?>
												<a href="<?php echo get_permalink( $prev_post->ID ); ?>" class="button solid-button dark"><?php _e('Prev ', 'Sivi') ?></a>
											<?php endif; ?>
											<?php $next_post = get_next_post();
											if (!empty( $next_post )): ?>
												<a href="<?php echo get_permalink( $next_post->ID ); ?>" class="button solid-button purple"><?php _e('Next ', 'Sivi') ?></a>
											<?php endif; ?>
										</p>
									</div>
								</div>
										...
										<div class="blog-video-wrapper">  
											...
											if ($video): ?>
												<div class="full-video">
													<?php
														if ($video):
															$post_embed = $mn_embed->run_shortcode('[embed width="790" height="392"]'.$video.'[/embed]'); 
														else:
							...
							<div class="blog-details">
								...
								<?php if (!$nobio):?>
									...
									?>
									<div class="section">
										<h5><?php _e('Author Details', 'Sivi') ?></h5>
										<div class="author">
											<div class="image">
												<?php echo $photo; ?>
											</div>
