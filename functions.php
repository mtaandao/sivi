...
define('sivi_PLUGINS',  get_template_directory() . '/library/includes/' ); // Shortcut to the /addons/ directory
define('sivi_SHORTCODES',  get_template_directory() . '/library/functions/shortcodes/ui.php' ); // Shortcut to the /addons/ directory
...
function sivi_load_scripts(){
	mn_enqueue_script('jquery');
	mn_enqueue_script('bootstrap',  get_template_directory_uri(). '/js/bootstrap.min.js', array('jquery'), '3.0.1' );
	mn_enqueue_script('plugins', get_template_directory_uri() .'/js/plugins.js', array('jquery'), '3.2', true);
	mn_enqueue_script('Google-map-api',  'http://maps.google.com/maps/api/js?sensor=true');
	mn_enqueue_script('main-scripts', get_template_directory_uri() .'/js/scripts.js', array('jquery'), '3.2', true);
}

function sivi_load_styles(){
	mn_enqueue_style('plugins-styles',  get_template_directory_uri().'/css/plugins.css');
	mn_enqueue_style('material-icons',  get_template_directory_uri().'/fonts/material-design-iconic-font/css/material-design-iconic-font.min.css');
	mn_enqueue_style('ionicons',  get_template_directory_uri().'/fonts/ionicons/css/ionicons.min.css');
	mn_enqueue_style('main-styles', get_stylesheet_directory_uri().'/style.css');
	mn_enqueue_style('dynamic-styles',  get_template_directory_uri().'/css/dynamic-styles.php');
	// Add RTL styles if set
	$rtl =  weblusive_get_option('rtl_mode');
	if ($rtl){
		mn_enqueue_style('rtl-styles',  get_template_directory_uri().'/css/rtl.css');
	}
}

add_action( 'mn_enqueue_scripts', 'sivi_load_styles' );
add_action( 'mn_enqueue_scripts', 'sivi_load_scripts' );



/************************************************************/

add_action( 'after_setup_theme', 'sivi_setup' );
function sivi_setup() {
	/************** ADD SUPPORT FOR LOCALIZATION ***************/

	load_theme_textdomain( 'Sivi',  get_template_directory() . '/languages' );
	...
	/********* SUPPORT FOR POST FORMATS AND TITLE TAGS **********/
	
	add_theme_support('post-formats', array('image', 'gallery', 'video', 'audio', 'link', 'quote') );
	add_theme_support( "title-tag" );
	if ( ! function_exists( '_mn_render_title_tag' ) ) {
		function theme_slug_render_title() {
			?><title><?php mn_title( '|', true, 'right' ); ?></title><?php
		}
		add_action( 'mn_head', 'theme_slug_render_title' );
	}
	...
	function my_custom_menus() {
		register_nav_menus(
			array(
				'primary_nav' => __( 'Primary Navigation', 'Sivi')
			)
		);
	}
	...
	?>
		<?php if(!$hidecomments): ?>
		 <?php if( 'open' == $post->comment_status && !$hidecomments) : ?>
			<?php comments_popup_link( __( '0 Comment ', 'Sivi' ), __( '1 comment ', 'Sivi' ), __( '% Comments ', 'Sivi' )); ?>
		 <?php endif?>
		<?php endif; ?>
		<?php if(!$hideauthor): ?>
			<?php _e('| By ', 'Sivi') ?><?php the_author_posts_link(); ?>
		<?php endif?>
...
if (is_admin() && isset($_GET['activated'])){
	mn_redirect(admin_url('admin.php?page=panel'));
}
...
add_action( 'widgets_init', 'sivi_widgets_init' );
function sivi_widgets_init() {
	...
	register_sidebar( array(
		'name' =>  __( 'Primary Widget Area', 'Sivi' ),
		'id' => 'primary-widget-area',
		'description' => __( 'The Primary widget area', 'Sivi' ),
...
add_action('login_head',  'sivi_login_logo'); 
...
function sivi_custom_gravatar ($avatar) {
...
add_filter( 'avatar_defaults', 'sivi_custom_gravatar' ); 
...
function sivi_custom_tag_cloud_widget($args) {
	$args['number'] = 0; //adding a 0 will display all tags
	$args['largest'] = 18; //largest tag
	$args['smallest'] = 12; //smallest tag
	$args['unit'] = 'px'; //tag font unit
	$args['format'] = 'list'; //ul with a class of mn-tag-cloud
	return $args;
}
add_filter( 'widget_tag_cloud_args', 'sivi_custom_tag_cloud_widget' );
...
function sivi_fill_widget_title($title){
	if( empty( $title ) )
		return '';
	else return $title;
}
add_filter('widget_title', 'sivi_fill_widget_title');
...
function my_theme_register_required_plugins() {
    ...
    $plugins = array(
		...
		array(
            ...
            'source'             => 'http://avendor.weblusive-themes.com/js_composer.zip', // The plugin source.
		...
		array(
            'name'               => 'Sivi Portfolio Plugin', // The plugin name.
            'slug'               => 'sivi-portfolio', // The plugin slug (typically the folder name).
            'source'             => get_stylesheet_directory() . '/installer/plugin-activation/plugins/sivi-portfolio.zip', // The plugin source.
		...
		array(
            'name'               => 'Sivi Slider Plugin', // The plugin name.
            'slug'               => 'sivi-slider', // The plugin slug (typically the folder name).
            'source'             => get_stylesheet_directory() . '/installer/plugin-activation/plugins/sivi-slider.zip', // The plugin source.
            'required'           => true, // If false, the plugin is only 'recommended' instead of required.
            'version'            => '1.0.0', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
            'force_activation'   => true, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
            'force_deactivation' => true, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
    ...
    $config = array(
        ...
        'strings'      => array(
            'page_title'                      => __( 'Install Required Plugins', 'Sivi' ),
            'menu_title'                      => __( 'Install Plugins', 'Sivi' ),
            'installing'                      => __( 'Installing Plugin: %s', 'Sivi' ), // %s = plugin name.
            'oops'                            => __( 'Something went wrong with the plugin API.', 'Sivi' ),
            ...
            'return'                          => __( 'Return to Required Plugins Installer', 'Sivi' ),
            'plugin_activated'                => __( 'Plugin activated successfully.', 'Sivi' ),
            'complete'                        => __( 'All plugins installed and activated successfully. %s', 'Sivi' ), // %s = dashboard link.
...
function vc_enqueue_main_css_forever() { 
	mn_enqueue_style('js_composer_front'); 
} 
if(function_exists('vc_map')){
	...
	add_action('mn_enqueue_scripts', 'vc_enqueue_main_css_forever');

}
/*********************widgets**********/
add_filter('mn_list_categories', 'cat_count_span');
...
add_filter('mn_get_attachment_link', 'rc_add_rel_attribute');
