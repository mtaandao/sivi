...
<?php while ( have_posts() ) : the_post(); 
			...
			<div class="content">
				...
				<?php else:?>
					<div class="alert alert-danger fade in">
						<?php _e('Audio post format was chosen but no embed code provided. Please fix this by providing it.', 'Sivi') ?>
					</div>
				<?php endif?>
				<footer>
					<span class="comments-link">
	...
	<div <?php post_class('blog-post post-format-quote'); ?>>
		<div class="inner">
			<div class="content">
				...
				<?php else:?>
					<div class="alert alert-danger fade in">
						<?php _e('Quote post format was chosen but no url or embed code provided. Please fix this by providing it.', 'Sivi') ?>
					</div>
				<?php endif?>
				<footer>
					<span class="comments-link">
				...
				<?php
					...
					<div class="row post-nomargin">
						<div class="full-video post-nopaddaing <?php if($image_pos=='image-top'):?>col-md-12<?php else:?>col-md-6<?php endif;?>">
							<?php
							if ($video):
								$post_embed = $mn_embed->run_shortcode('[embed width="870"]' . $video . '[/embed]');
							else:
						...
						<?php else: ?>
						<div class="alert alert-danger fade in">
						<?php _e('Video post format was chosen but no url or embed code provided. Please fix this by providing it.', 'Sivi') ?>
						</div>
					<?php endif;?>
				<?php endif ?>
	...
	<div <?php post_class("blog-post post-format-standard $image_pos"); ?>>
		<div class="inner">
			<?php
			if($image_pos!=='image-none'): 
				$image_url = mn_get_attachment_url( get_post_thumbnail_id($post->ID) );
				if($image_url):?>
					<a href="<?php the_permalink()?>">
						<div class="image"><?php echo $thumbnail?></div>
					</a>
