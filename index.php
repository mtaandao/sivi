...
<?php get_header(); 
	...
	$menu_items = mn_nav_menu( array('container' => false,'items_wrap' => '%3$s', 'depth' => 0, 'menu' =>'primary_nav', 'theme_location' =>'primary_nav', 'echo'  => false,'walker' => $walker));
...
<div class="sections">
	<div class="sections-wrapper clearfix onepage">
		...
		foreach ( $pages as $pag ) {
			...
			$template = get_post_meta( $page->ID, '_mn_page_template', true );
			...
			if ($pageType == '1')
			{
				switch ($template)
				{
					case 'homepage-template.php':
						...
						?>
						<section id="<?php echo esc_attr($slug)?>" class="sivi-homepage no-padding-bottom imagepar <?php echo $nobottom.$notop; ?>  waypoint pagecustom-<?php echo $page->ID?>">
							<div class="container">
					...
					case 'portfolio-template.php':
						$page_template_name = get_post_meta( $page->ID,'_mn_page_template',true); 
						$content = $page->post_content;
						$pageId = $page->ID;
						$itemsize = '263x351';	
							...
							<div class="portfolio-wrapper">
								<div class="portfolio-filters" id="portfolio-filters">
										<ul data-option-key="filter" class="folio-filter xtra non-paginated">
											<li><a href="#" class="button solid-button white-purple small"><?php _e('All', 'Sivi')?></a></li>
											<?php 
												...
												$categories = mn_list_categories ($args);
											?>
										</ul>
								</div>
								<div class="portfolio-grid portfolio" id="portfolio">									
											<?php 
											$items_per_page = 777;
											$loop = new MN_Query(array('post_type' => 'portfolio', 'posts_per_page' => $items_per_page)); 
											$cats = get_post_meta($pageId, "_page_portfolio_cat", $single = true);
											if( $cats == '' ): ?>
												<p><?php _e('No categories selected. Please login to your MN Admin area and set
													the categories you want to show by editing this page and setting one or more categories 
													in the multi checkbox field "Portfolio Categories".', 'Sivi')?>
												</p>
											<?php else: ?>		
												<?php 
												 ...
												 <?php if (!empty($portfolio_posts_to_query)):			
													$mn_query = new MN_Query( array( 'post_type' => 'portfolio', 'orderby' => 'menu_order', 'order' => 'ASC', 'post__in' => $portfolio_posts_to_query, 'paged' => $paged, 'shomnosts' => $items_per_page) ); 
													
													if ($mn_query->have_posts()):  ?>
													<?php $counter=1; while ($mn_query->have_posts()) : 							
														$mn_query->the_post();
														$custom = get_post_custom($post->ID);
															 
														// Get the portfolio item categories
														$cats = mn_get_object_terms($post->ID, 'portfolio_category');
														...
														elseif (isset($custom['_portfolio_no_lightbox'][0]) && $custom['_portfolio_no_lightbox'][0] != '') { 
															...
															$full_image = mn_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'full', false);
															$link = $full_image[0];				
														}
						...
						<section id="<?php echo esc_attr($slug)?>" class="imagepar blog-section blog-mini <?php echo $nobottom.$notop; ?> pagecustom-<?php echo $page->ID?>" <?php if (isset($customBg) && !empty($customBg[0])):?>style="background-image:url(<?php echo $customBg[0]?>)"<?php endif?>>
							...
							<div class="container">
								...
								<div class="blog-page" id="ajax-load-more">
									<div class="blog-posts masonry" id="blog-masonry" data-path="<?php echo get_template_directory_uri(); ?>" data-post-type="post" data-category="" data-taxonomy="" data-tag="" data-author="" data-display-posts="<?php echo $postlimit ?>" data-button-text="<?php _e('Load More Posts', 'Sivi' ); ?>">
										<div class="blog-grid-sizer"></div>
										<?php if ( $mn_query->max_num_pages > 1 ): ?><?php endif?>
									</div>
